...
{
	const v3 = new Vector2D(0, 5).normalize();
...
{
	const v4 = new Vector2D(2, -5).rotate(4 * Math.PI);
	...
	for (const expectedVector of unitCircle)
	{
		const computedVector = new Vector2D(1, 0).rotate(-expectedVector.angle);
...
// Test Vector2D rotation further
{
	...
	for (let i = 0; i <= 128; ++i)
	{
		...
		const vec1 = new Vector2D(Math.cos(angle), Math.sin(angle));
		const vec2 = new Vector2D(1, 0).rotate(-angle);

		TS_ASSERT_EQUALS_APPROX(vec1.x, vec2.x, epsilon);
		TS_ASSERT_EQUALS_APPROX(vec1.y, vec2.y, epsilon);

		const vec3 = new Vector2D(Math.sin(angle), Math.cos(angle));
		const vec4 = new Vector2D(0, 1).rotate(angle);
...
{
	const epsilon = 0.00000001;
	const v1 = new Vector2D(-4, 8).rotateAround(Math.PI / 3, new Vector2D(-1, -3));
...
{
	const v7 = new Vector2D(4, 5).perpendicular();
	TS_ASSERT_EQUALS(v7.x, -5);
	TS_ASSERT_EQUALS(v7.y, 4);

	const v8 = new Vector2D(0, 0).perpendicular();
...
// Test Vector2D list functions
{
	...
	const sum = Vector2D.sum(list);
	TS_ASSERT_EQUALS(sum.x, 143);
	TS_ASSERT_EQUALS(sum.y, -52);

	const avg = Vector2D.average(list);
...
// Test Vector2D round
{
	const v1 = new Vector2D(-4.5, 8.2).round();
	TS_ASSERT_EQUALS(v1.x, -4);
	TS_ASSERT_EQUALS(v1.y, 8);

	const v2 = new Vector2D(NaN, NaN).round();
	TS_ASSERT(isNaN(v2.x));
	TS_ASSERT(isNaN(v2.y));

	const v3 = new Vector2D().round();
	TS_ASSERT_EQUALS(v3.x, 0);
	TS_ASSERT_EQUALS(v3.y, 0);
}
...
// Test Vector2D floor
{
	const v1 = new Vector2D(-4.5, 8.9).floor();
	TS_ASSERT_EQUALS(v1.x, -5);
	TS_ASSERT_EQUALS(v1.y, 8);

	const v2 = new Vector2D().floor();
...
// Test Vector3D mult
{
	const v2 = new Vector3D(2, 5, 14).mult(3);
...
{
	const v5 = new Vector3D(1, 2, 3).cross(new Vector3D(4, 5, 6));
...
// Test Vector3D round
{
	const v1 = new Vector3D(-1.1, 2.2, 3.3).round();
	TS_ASSERT_EQUALS(v1.x, -1);
	TS_ASSERT_EQUALS(v1.y, 2);
	TS_ASSERT_EQUALS(v1.z, 3);

	const v2 = new Vector3D(NaN, NaN, NaN).round();
	TS_ASSERT(isNaN(v2.x));
	TS_ASSERT(isNaN(v2.y));
	TS_ASSERT(isNaN(v2.z));

	const v3 = new Vector3D().round();
	TS_ASSERT_EQUALS(v3.x, 0);
	TS_ASSERT_EQUALS(v3.y, 0);
	TS_ASSERT_EQUALS(v3.z, 0);

	const v4 = new Vector3D(71.8, 73.4, 73.89).round();
...
// Test Vector3D floor
{
	const v1 = new Vector3D(-1.1, 2.2, 3.9).floor();
	TS_ASSERT_EQUALS(v1.x, -2);
	TS_ASSERT_EQUALS(v1.y, 2);
	TS_ASSERT_EQUALS(v1.z, 3);

	const v3 = new Vector3D().floor();
	TS_ASSERT_EQUALS(v3.x, 0);
	TS_ASSERT_EQUALS(v3.y, 0);
	TS_ASSERT_EQUALS(v3.z, 0);
}
