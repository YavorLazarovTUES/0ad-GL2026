 ...
 */
std::string ToString(const Script::Request& rq, JS::MutableHandleValue obj, bool pretty = false);
 ...
 */
bool ParseJSON(const Script::Request& rq, const std::string& string_utf8, JS::MutableHandleValue out);

/**
 * Read a JSON file. Returns the unmodified value on error and prints an error message.
 */
void ReadJSONFile(const Script::Request& rq, const VfsPath& path, JS::MutableHandleValue out);

/**
 * Stringify to a JSON string, UTF-8 encoded. Returns an empty string on error.
 */
std::string StringifyJSON(const Script::Request& rq, JS::MutableHandleValue obj, bool indent = true);
}
